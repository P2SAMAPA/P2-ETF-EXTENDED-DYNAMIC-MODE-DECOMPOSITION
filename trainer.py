...
import data_manager
import push_results
from edmd_engine import compute_edmd_scores
...
def main():
    ...
    macro_df = data_manager.prepare_macro(df)   # loaded for consistency; EDMD itself is univariate
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (EDMD Engine) ===")
        ...
        for win in config.WINDOWS:
            min_bars = win + config.DELAY_DIM + config.N_RBF_CENTERS + 10
        ...
        for win, score_records in window_results.items():
            ...
            windows_tab2[str(win)] = {
                ...
                "full_ranking": [
                    [t, rec["score"], rec["forecast_signal"], rec["growth_rate"], rec["fit_quality"]]
                    for t, rec in sw
                ],
            }
    ...
    print(f"\n=== EDMD Engine complete: {tab1_path.name}, {tab2_path.name} ===")
